...
import { BUNDLED_VOICE, humanVoice, MAX_VOICE_BYTES, VOICE_SCRIPT, voiceFormat } from '../audio/HumanVoice';

export function VoiceStudio()
{
    const [selected, setSelected] = useState(() => (VOICE_SCRIPT.find(item => !humanVoice.has(item.id)) ?? VOICE_SCRIPT[0]).id);
    ...
    const save = async (remove = false) => {
        ...
        try {
            ...
            setMessage(remove ? BUNDLED_VOICE[selected] ? 'Voz incluída restaurada.' : 'Gravação removida.' : next ? 'Trecho salvo. Vamos gravar a próxima fala que falta.' : 'A narração está completa neste aparelho. Você já pode ouvi-la no jogo.');
        } catch (error) { if (active.current) setMessage((error as Error).message); }
        finally { if (active.current) setBusy(false); }
    };
    ...
    const importPack = async (file: File) => {
        ...
        try {
            ...
            setMessage(`${count} trechos carregados. ${next ? 'Os demais trechos foram mantidos.' : 'A narração está pronta neste aparelho.'}`);
        } catch (error) { if (active.current) setMessage((error as Error).message); }
        finally { if (active.current) setBusy(false); }
    };
    ...
    return <section className="voice-studio" aria-labelledby="voice-studio-title">
        ...
        <select id="voice-line" value={selected} disabled={recording || busy} onChange={(event) => { stopPreview(); setSelected(event.target.value); setDraft(undefined); setMessage(''); }}>
            {VOICE_SCRIPT.map((item) => <option key={item.id} value={item.id}>{humanVoice.hasCustom(item.id) ? 'Sua voz · ' : humanVoice.has(item.id) ? 'Incluída · ' : 'Gravar · '}{item.label}</option>)}
        </select>
        ...
        {!draft && humanVoice.hasCustom(selected) && <button className="text-button" disabled={busy || recording} onClick={() => save(true)}>{BUNDLED_VOICE[selected] ? 'Restaurar voz incluída' : 'Apagar este trecho'}</button>}
